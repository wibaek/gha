...
`ssh-compose-vps-deploy.yaml`과 `ssh-compose-image-load-deploy.yaml`은 SSH로 서버에 접속해서 Docker Compose를 실행합니다.
...
같은 repository/package에 연결된 private GHCR image는 caller deploy job에 `packages: read` 권한을 주면 별도 PAT 없이 pull할 수 있습니다.
다른 repository/package의 private image를 pull해야 하면 package access를 workflow repository에 부여하거나, 필요한 경우에만 `GHCR_TOKEN` override를 사용합니다.

`ssh-compose-vps-deploy.yaml`과 `ssh-compose-image-load-deploy.yaml`은 `PROD_APP_ENV` 값을 서버의 `/srv/my-app/app.env`로 매 배포마다 갱신합니다.
`app.env` 파일은 repository에 커밋하지 않습니다.
workflow는 업로드한 `app.env`와 `compose.env` 파일 권한을 `600`으로 고정합니다.

```bash
sudo install -m 600 -o deploy -g deploy /dev/null /srv/my-app/app.env
```
